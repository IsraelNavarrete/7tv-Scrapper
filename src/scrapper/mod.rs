...
//TODO falta arreglar que cree el directorio
async fn download_image(emote_data: Option<(String, String)>, client: Client) {
    ...
    println!("Se ha descargado la imagen en: {}",emote_path.to_str().unwrap());
}
...
async fn handle_response(
    ...
    match response.status() {
        ...
        StatusCode::OK => {
            ...
            for image in images_emote {
                if is_animated_and_has_correct_size(size.clone(), &image) {
                    return Some((emote_name.clone(), image.url.clone()))
                } else if has_correct_size(size.clone(), &image) {
                    return Some((emote_name.clone(), image.url.clone()))
                }
            }

            println!("No había imagenes para: {}", emote_name);
...
fn has_correct_size(size: String, image: &Image) -> bool {
    image.mime.contains("png") && is_correct_scale(size, image)
}
...
fn is_animated_and_has_correct_size(size: String, image: &Image) -> bool {
    image.frame_count > 1
        && image.mime.contains("gif")
        && image.scale
            == size
                .chars()
                .next()
